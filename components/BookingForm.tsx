...
import type { PricingData, ServiceItem, BookingData, ServiceKey } from '@/types'
import ServiceItemRow from './ServiceItemRow'
import SummarySidebar from './SummarySidebar'
import BookingModal from './BookingModal'

function newItem(sizeKey: string): ServiceItem {
  return { id: Math.random().toString(36).slice(2), type: 'wash', sizeKey }
}
...
export default function BookingForm({ initialPricing }: Props) {
  ...
  const defaultSizeKey = pricing.sizes[0]?.key ?? ''
  const [items, setItems] = useState<ServiceItem[]>([newItem(defaultSizeKey)])
  ...
  const rateLoaded = pricing.source.rate
  ...
  async function handleRefresh() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPricing(data)
        // Reset items to use first available size from updated pricing
        setItems([newItem(data.sizes[0]?.key ?? '')])
      }
    } catch {}
    setRefreshing(false)
  }

  function handleItemChange(id: string, field: 'type' | 'sizeKey', value: string) {
    setItems(prev =>
      prev.map(item =>
        item.id === id
          ? { ...item, [field]: value as ServiceKey }
          : item
      )
    )
  }
  ...
  function handleAddItem() {
    setItems(prev => [...prev, newItem(pricing.sizes[0]?.key ?? '')])
  }

  function handleReset() {
    setItems([newItem(pricing.sizes[0]?.key ?? '')])
    setName(''); setPhone(''); setAddress('')
    setDate(todayStr()); setTime('09:00'); setDistIdx(0); setError('')
  }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!rateLoaded)    { setError('ยังไม่ได้โหลดราคาจาก Google Sheet กรุณากด ↻ แล้วลองใหม่'); return }
    ...
    const enrichedItems = items.map(item => ({
      ...item,
      price: pricing.prices[item.type]?.[item.sizeKey] ?? 0,
    }))
    const subtotal = enrichedItems.reduce((s, i) => s + (i.price ?? 0), 0)
  ...
  return (
    <>
      {/* Rate load error banner */}
      {!rateLoaded && (
        <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-[8px] px-4 py-3 mb-6 text-sm text-red-700">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
            <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
          </svg>
          <span>ไม่สามารถโหลดราคาจาก Google Sheet ได้ — กด ↻ ในแถบขวาเพื่อลองใหม่ หรือตรวจสอบว่า Sheet ถูก Publish แล้ว</span>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="flex-[2] space-y-6">
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              <div className="hidden sm:flex gap-3 text-xs font-medium text-muted">
                ...
                <select className="field-input" value={distIdx} onChange={e => setDistIdx(Number(e.target.value))}>
                  {pricing.distanceOptions.map((opt, i) => (
                    <option key={i} value={i}>
                      {opt.label}{opt.fee > 0 ? ` (+฿${opt.fee.toLocaleString()})` : ' (ฟรี)'}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button
              ...
              disabled={!rateLoaded}
              style={{ opacity: rateLoaded ? 1 : 0.5, cursor: rateLoaded ? 'pointer' : 'not-allowed' }}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
